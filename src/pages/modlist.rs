...
use crate::utils::helper::is_installed;
          ...
          section class="imports" {
            div class="row even" {
              @if !modlist.is_packed() {
                form method="post" action="/api/modlist/merge" {
                  input type="hidden" name="modlist_name" value=(modlist.name);
                  input id="merge" type="submit" value="merge" class="text-style" title=(merge_help);
                }
              }
              ...
              @if modlist.is_packed() {
                form method="post" action="/api/modlist/unpack" {
                  input type="hidden" name="modlist_name" value=(modlist.name);
                  input id="unpack" type="submit" value="unpack" class="text-style" title=(unpacking_help);
                }
              } @else {
                @if does_import_a_modlist {
                  form {
                    input id="pack" type="submit" value="pack" class="text-style" disabled="true" title=(packing_blocked_help);
                  }
                } @else {
                  form method="post" action="/api/modlist/pack" {
                    input type="hidden" name="modlist_name" value=(modlist.name);
                    input id="pack" type="submit" value="pack" class="text-style" title=(packing_help);
                  }
                }
              }
            }

            form class="import" method="post" action="/api/modlist/import" {
              fieldset {
                ...
                div class="row even" {
                  select title="select modlist" name="imported_name" required {
                  ...
                  input id="import" class="text-style" type="submit" value="import";
                }
              }
            }
            ...
            div class="row even" {
              form method="post" action="/api/modlist/load-imports" {
                input type="hidden" name="modlist_name" value=(modlist.name);
                input id="load" class="text-style" loaded=(modlist.loaded) installed=(is_installed(&modlist.name)) merged=(modlist.is_merged()) type="submit" value="load imports & install";
              }

              form method="post" action="/api/modlist/unload-imports" {
                input type="hidden" name="modlist_name" value=(modlist.name);
                input id="unload" class="text-style" type="submit" value="unload imports";
              }
            }
                ...
                li {
                  ...
                  span class="row" {
                    ...
                    form method="post" action="/api/modlist/remove-import" {
                      ...
                      input type="submit" class="text-style" value="remove";
                    }
                  }
                }
              }
...
fn get_javascript() -> String {
  ...
  const unpack    = document.getElementById('unpack');
  const load      = document.getElementById('load');
  const unload    = document.getElementById('unload');
  const imp       = document.getElementById('import');
  const ulElement = document.getElementById('list');
  const inputs    = ulElement.querySelectorAll('input[type="submit"]');
  const loaded    = JSON.parse(load.getAttribute('loaded'));
  const installed = JSON.parse(load.getAttribute('installed'));
  const merged    = JSON.parse(load.getAttribute('merged'));
  ...
  if (items.length == 0) {
    ...
    if (!loaded) {
      unload.disabled = true;
      merge.disabled = true;
      merge.title = 'Load first';
      pack.disabled = true;
    }else{
      load.disabled = true;
      load.title = 'Unload first';
      imp.disabled = true;
      imp.title = 'Unload first';  
      inputs.forEach( input => { input.disabled = true; input.title = 'Unload first';});
    }    

    
    for (let i = 0; i < items.length; i++) {
      const forms = items[i].getElementsByTagName('form');
      for (let j = 0; j < forms.length; j++) {      
        let inputs = forms[j].getElementsByTagName('input');
        let inputsToHide = Array.from(inputs).filter(input => input.classList.contains('rotate-90-clockwise'));
        if (inputs[1].value === 'vanilla') {
    ...
    if (!installed) {
      ...
      merge.title = 'Load first';
      if (loaded) {
        merge.title = 'Install first on main page';
      }
    }
  }  
